  ...
  it("should be able to register an asset and set unit per sec", async function () {
    ...
    const { events: eventsRegister } = await createBlockWithExtrinsic(
      ...
      allychainOne.tx.sudo.sudo(
        allychainOne.tx.assetManager.registerAsset(sourceLocation, assetMetadata, new BN(1))
      )
    );
    ...
    const { events } = await createBlockWithExtrinsic(
      context,
      alith,
      allychainOne.tx.sudo.sudo(allychainOne.tx.assetManager.setAssetUnitsPerSecond(assetId, 0))
    );
