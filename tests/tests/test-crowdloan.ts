...
describeDevMoonbeam("Crowdloan", (context) => {
  ...
  it("should be able to make a first claim", async function () {
    ...
    const account = await context.axiaApi.query.system.account(GENESIS_ACCOUNT);
...
describeDevMoonbeam("Crowdloan", (context) => {
  ...
  it("should be able to initialize through democracy", async function () {
    ...
    // Propose
    await context.axiaApi.tx.democracy.propose(encodedHash, 1000n * GLMR).signAndSend(sudoAccount);
