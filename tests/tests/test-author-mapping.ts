...
describeDevMoonbeam("Author Mapping - simple association", (context) => {
  it("should match genesis state", async function () {
    ...
    expect((await context.axiaApi.query.system.account(ALITH)).data.free.toBigInt()).to.eq(
      1207825819614629174706176n
    );
    expect((await context.axiaApi.query.system.account(ALITH)).data.reserved.toBigInt()).to.eq(
  ...
  it("should succeed in adding an association", async function () {
    ...
    const { events } = await createBlockWithExtrinsic(
      context,
      genesisAccount,
      context.axiaApi.tx.authorMapping.addAssociation(bobAuthorId)
    );
    // check events
    expect(events.length === 8);
    expect(context.axiaApi.events.balances.Reserved.is(events[1])).to.be.true;
    expect(context.axiaApi.events.authorMapping.AuthorRegistered.is(events[2])).to.be.true;
    expect(context.axiaApi.events.system.NewAccount.is(events[4])).to.be.true;
    expect(context.axiaApi.events.balances.Endowed.is(events[5])).to.be.true;
    expect(context.axiaApi.events.treasury.Deposit.is(events[6])).to.be.true;
    expect(context.axiaApi.events.system.ExtrinsicSuccess.is(events[7])).to.be.true;
...
describeDevMoonbeam("Author Mapping - Fail to reassociate alice", (context) => {
  it("should fail in adding an association for a second time", async function () {
    ...
    const { events } = await createBlockWithExtrinsic(
      context,
      baltathar,
      context.axiaApi.tx.authorMapping.addAssociation(aliceAuthorId)
    );
    ...
    //check state
    expect((await context.axiaApi.query.system.account(BALTATHAR)).data.free.toBigInt()).to.eq(
      1208925818354628455674176n
    );
    expect((await context.axiaApi.query.system.account(BALTATHAR)).data.reserved.toBigInt()).to.eq(
      0n
    );
    expect((await getMappingInfo(context, aliceAuthorId)).account).to.eq(ALITH);
  });
...
describeDevMoonbeam("Author Mapping - Fail without deposit", (context) => {
  before("setup association", async function () {
    ...
    expect((await context.axiaApi.query.system.account(RANDOM_ADDRESS)).data.free.toBigInt()).to.eq(
      0n
    );
    try {
      await context.axiaApi.tx.authorMapping.addAssociation(bobAuthorId).signAndSend(rando);
    } catch (e) {
...
describeDevMoonbeam("Author Mapping - registered author can clear (de register)", (context) => {
  it("should succeed in clearing an association", async function () {
    ...
    const { events } = await createBlockWithExtrinsic(
      context,
      genesisAccount,
      context.axiaApi.tx.authorMapping.clearAssociation(bobAuthorId)
    );
    //check events
    expect(events.length === 6);
    expect(context.axiaApi.events.balances.Unreserved.is(events[1])).to.be.true;
    expect(context.axiaApi.events.authorMapping.AuthorDeRegistered.is(events[2])).to.be.true;
    expect(context.axiaApi.events.treasury.Deposit.is(events[4])).to.be.true;
    expect(context.axiaApi.events.system.ExtrinsicSuccess.is(events[5])).to.be.true;
...
describeDevMoonbeam("Author Mapping - non author clearing", (context) => {
  it("should not succeed in clearing an association for a non-author", async function () {
    ...
    expect(context.axiaApi.events.treasury.Deposit.is(events[2])).to.be.true;
    expect(context.axiaApi.events.system.ExtrinsicFailed.is(events[3])).to.be.true;
  });
});
