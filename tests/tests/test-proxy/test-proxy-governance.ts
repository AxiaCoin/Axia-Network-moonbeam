...
describeDevMoonbeam("Proxing governance", (context) => {
  ...
  it("should be able to vote on behalf of the delegate account", async function () {
    ...
    const dorothyPreBalance = (
      await context.axiaApi.query.system.account(DOROTHY)
    ).data.free.toBigInt();
    ...
    expect(context.axiaApi.events.proxy.ProxyExecuted.is(events[1])).to.be.true;
    expect(events[1].data[0].toString()).to.equal("Ok");
    expect(context.axiaApi.events.treasury.Deposit.is(events[3])).to.be.true;
    ...
    let dorothyAccountData = await context.axiaApi.query.system.account(DOROTHY);
    ...
    const referendumInfoOf = (
      await context.axiaApi.query.democracy.referendumInfoOf(0)
    ).unwrap() as any;
